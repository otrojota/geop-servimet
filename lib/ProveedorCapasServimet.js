...
class ProveedorCapasServimet extends ProveedorCapas {
        ...
        config.estaciones.forEach(e => {
            ...
            let extraConfig = {
                configAnalisis:{analizador:"serie-tiempo", analizadores:{
                    "serie-tiempo":{
                        variable:"servimet.ESTACIONES_SERVIMET.${codigo-objeto}.TEMP", nivelVariable:0,
    ...
    async generaSerieTiempoEstacionesServimet(args, cod) {
        try {
            ...
            if (!rows.length) throw "No hay Datos para el Período." + (ajusto?" El período original era muy amplio y se ajustó a 20 días desde el inicio sollicitado.":"");
            let ret = {
